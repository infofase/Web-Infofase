...
def download_binary_csv():
    """Descarga el CSV de Binary Canarias. Devuelve el texto o None."""
    if not BINARY_CSV_URL:
        return None
    ...
    try:
        ...
        for enc in ('utf-8-sig', 'utf-8', 'latin-1', 'cp1252'):
            try:
                text = raw.decode(enc)
                log(f"    Binary CSV: {len(raw)//1024}KB ({enc})")
                return text
            except UnicodeDecodeError:
                continue
    except URLError as e:
        log(f"    ERROR descargando Binary CSV: {e}")
    return None
