...
def get_icecat_img(brand, product_code, cache):
    ...
    Usa cache para no repetir llamadas ya resueltas.
    """
    ...
    # Normalizar brand para Icecat (capitalizar primera letra)
    ...
    try:
        req = Request(url, headers={
            "User-Agent": "Infofase-Bot/3.0",
            "Authorization": f"Basic {base64.b64encode(f'{ICECAT_USER}:{ICECAT_PASS}'.encode()).decode()}"
        })
        with urlopen(req, timeout=8) as r:
            data = json.loads(r.read().decode("utf-8", errors="replace"))

        # Navigate Icecat JSON response structure
        msg = data.get("msg", {})
        if not msg:
        ...
        thumb = img_data.get("ThumbPic", "")
        high  = img_data.get("HighPic", "")

        # Fallback to gallery images if no main image
        if not thumb:
            gallery = msg.get("Gallery", [])
            if gallery:
                first = gallery[0]
                thumb = first.get("ThumbPic", "")
                high  = first.get("HighPic",  "")
        ...
        if e.code == 404:
            cache[cache_key] = None  # producto no encontrado — cachear para no volver a buscar
        # Other errors: don't cache, may be temporary
        return None, None
    except Exception:
        return None, None
...
def main():
    ...
    log(f"  Cache de imágenes: {len(img_cache)} entradas")
