...
Actualiza los productos dentro del _TG (tienda iframe) en la web integrada.
...
_csv_rows = []  # set in main(), used by update_zona_apple
...
def categorize(producto):
    ...
    if PRODUCTO_CAT.get(p) is None and p in PRODUCTO_CAT:
        return None
...
def ascii_encode(html_str):
    """Convierte caracteres no-ASCII a entidades HTML para base64 seguro"""
    out = []
    for ch in html_str:
        if ord(ch) > 127:
            out.append(f'&#{ord(ch)};')
...
def update_html(products):
    ...
    # Encode new products as UTF-8 base64
    new_prods_b64 = base64.b64encode(
        json.dumps(products, ensure_ascii=False,
                   separators=(",",":")).encode("utf-8")
    ).decode("ascii")

    # ── Strategy A: direct var ALL pattern (tienda standalone) ──
    pat_direct = r'((?:var ALL = JSON\.parse\(new TextDecoder\(\)\.decode\(Uint8Array\.from\(atob\(|var ALL = JSON\.parse\(atob\()")[A-Za-z0-9+/=]+'
    html2, n = re.subn(
        pat_direct,
    ...
    if n > 0:
        ...
        # ── Strategy B: products are inside _TG (integrated site) ──
        tg_m = re.search(r"(var _TG = ')([A-Za-z0-9+/=]+)(')", html)
        if not tg_m:
            log("ERROR: no se encontró _TG ni var ALL en template.html"); return False

        log("  Modo: web integrada (_TG)")
        tg_b64  = tg_m.group(2)
        tg_html = base64.b64decode(tg_b64).decode("ascii", errors="replace")

        # Replace products b64 inside the tienda iframe HTML
        pat_inner = r'((?:JSON\.parse\(new TextDecoder\(\)\.decode\(Uint8Array\.from\(atob\(|JSON\.parse\(atob\()")[A-Za-z0-9+/=]+'
        tg_new, n2 = re.subn(
            pat_inner,
            lambda m: m.group(1) + '"' + new_prods_b64,
            tg_html, count=1)
        ...
        if n2 == 0:
            log("ERROR: patrón de productos no encontrado dentro de _TG"); return False

        # Re-encode _TG with ASCII-safe encoding
    ...
    # ── Update Zona Apple prices & stock ──────────────────────
    ...
    log(f"  {OUTPUT}: {size_mb:.1f}MB, {count} productos tienda + {za_updated} Apple")
    return True
...
# ══════════════════════════════════════════════════════════════
# ZONA APPLE — actualizar precios y stock dentro de _ZA
# ══════════════════════════════════════════════════════════════
def update_zona_apple(html, csv_rows):
    """
    Actualiza price, canon, status, stock, transit en los 593 productos
    de la Zona Apple usando los datos del CSV de Megastore.
    Las imágenes y categorías NO se tocan.
    """
    # Build CSV lookup by product id
    csv_by_id = {}
    for row in csv_rows:
        pid = row.get('codigo','').strip().lower()
        if pid:
            csv_by_id[pid] = row

    # Find _ZA in the HTML
    ...
    # Find and parse the ALL array
    all_idx = za_html.find('let ALL    = [')
    if all_idx < 0:
        all_idx = za_html.find('let ALL = [')
    if all_idx < 0:
        log("  ZA: let ALL no encontrado en _ZA")
        return html, 0

    all_end = za_html.find('];\n', all_idx)
    prefix  = za_html[:all_idx]
    suffix  = za_html[all_end+1:]
    key     = za_html[all_idx:za_html.index('[', all_idx)+1]  # 'let ALL    = ['

    try:
        za_products = json.loads(za_html[za_html.index('[', all_idx):all_end+1])
    except Exception as e:
        log(f"  ZA: error parseando productos: {e}")
        return html, 0
    ...
    log(f"  ZA: {updated}/{len(za_products)} productos actualizados")

    # Rebuild _ZA with updated products
    new_all = json.dumps(za_products, ensure_ascii=False, separators=(',',':'))
    new_za_html = prefix + key + new_all[1:] + suffix  # new_all[1:] removes leading [

    # ASCII-encode and re-encode to base64
    new_za_b64 = base64.b64encode(
        ascii_encode(new_za_html).encode('ascii')
    ).decode('ascii')

    new_html = html[:za_m.start(2)] + new_za_b64 + html[za_m.end(2):]
