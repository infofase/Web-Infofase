...
def get_icecat_img(brand, product_code, cache):
    ...
    Autenticación: HTTP Basic Auth con ICECAT_USER + ICECAT_PASS.
    ...
    # Usar la marca exactamente como viene del CSV — NO aplicar .title() porque
    # convierte "HP"→"Hp", "QNAP"→"Qnap", "WD"→"Wd", "LG"→"Lg" (incorrectos en Icecat)
    brand_enc = quote(brand.strip())
    code_enc  = quote(product_code.strip())
    url = (f"https://live.icecat.biz/api/"
           f"?UserName={ICECAT_USER}"
           f"&Language=EN"   # EN: disponible para todos los productos (ES causa 404 en muchos)
           f"&Brand={brand_enc}"
           f"&ProductCode={code_enc}")
    try:
        # Cuenta de pago: HTTP Basic Auth
        auth_str  = base64.b64encode(
            f"{ICECAT_USER}:{ICECAT_PASS}".encode()).decode()
        req = Request(url, headers={
            "User-Agent":    "Infofase-Bot/4.0",
        ...
        if thumb or high:
            ...
            # Sin imagen en Icecat — guardar None para no reintentar en próximas ejecuciones
            cache[cache_key] = None
            return None, None

    except HTTPError as e:
        # 404/403 = producto no encontrado en Icecat → cachear None, no reintentar
